...
def importZip(options):
    ...
    with open(US_ZIP_LIST, 'r', encoding="utf8") as f:
        reader = csv.reader(f)
        next(reader)
        for row in reader:
            #print('row',row,row[0],row[11])
            try:
...
def importGHCND(options):
    ...
    with open(GHCND_STATIONS_LIST) as f:
        for l in f:
            ...
            row = l.split(' ')
            try:
                c.execute("INSERT OR IGNORE INTO stations (id, name, state, lat, lon, elev, type) VALUES (?, ?, ?, ?, ?, ?, 'GHCND')", (row[0], name, state, float(row[1]), float(row[2]), float(row[3])))
            except:
                print("WARNING: Cannot insert GHCHD row ==> {0!s}".format(str(row)))
    conn.commit()
    conn.close()
...
def importISD(options):
    ...
    with open(ISD_STATIONS_LIST, 'r', encoding='utf8') as f:
