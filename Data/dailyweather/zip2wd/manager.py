...
def make_server_manager(ip, port, authkey):
    ...
    authkey = str.encode(authkey)
    print('key',ip,port,authkey)
    ...
    for infile in args.inputs:
        with open(infile, 'r') as csvfile:
            ...
            for r in reader:
                ...
                zips.append(data)
        N = len(zips)
        # print("number os zips find", zips)
        ...
        for i in range(0, len(zips), chunksize):
            print('Put task,{}'.format(i))
            shared_job_q.put(zips[i:i + chunksize])
        
        print('finished,N is {}, empty is {}'.format(N, shared_result_q.empty()))
        ...
        while numresults < N:
            try:
                if not shared_result_q.empty():
                    outdict = shared_result_q.get(timeout=2)
                    print('outputdict', outdict)
...
def main(args=None):
    if args is None:
        ...
        # args.config='zip2wd.cfg'
        # args.inputs=['sample-input-basic.csv']
        # args.outfile='output.csv'
        # args.verbose=False

        # args = argparse.Namespace(config='zip2wd.cfg', inputs=['sample-input-basic.csv'], outfile='output.csv', verbose=False)
